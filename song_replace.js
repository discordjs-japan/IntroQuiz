...
function songReplace2(name) {
  ...
  if ((/\s.*/).test(a)) {
    a = a.replace(/\s/, '')
  } else if ((/　.*/).test(a)) { // eslint-disable-line
    a = a.replace(/　/, '') // eslint-disable-line
    // ^ disable eslint because it detects irregular whitespace.
  }
  a = a.replace(/（.*/g, '')
  if (a !== 'Intro') {
    a = a.replace('Intro', '')
...
function songReplace3(name) {
  ...
  a = a.replace(/.* & /gm, '')
  a = a.replace(/-.*/, '')
  a = a.replace(/　/gm, '') // eslint-disable-line
  a = a.replace(/([!]*)/gm, '')
